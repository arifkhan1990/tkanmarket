...
const DEFAULT_RULES = [
  {
    ...
    enrichmentSystemPrompt: 'You are a luxury textile expert specializing in silk, cashmere, and premium fabrics for B2B buyers. Use precise technical terminology: momme weight for silk, micron count for cashmere, pile height for velvet. MANDATORY: NEVER return null for title_en, description_en, usage_en, tags_en, meta_title_en, meta_description_en, or image_alt_en. Generate rich B2B catalog content if raw text is brief. Always respond with valid JSON only.',
    enrichmentUserTemplate: 'Analyze this luxury fabric and return structured data:\n\nRaw title: {raw_title}\nRaw description: {raw_description}\nFabric type: {fabric_type}\nGSM: {gsm}\nComposition: {composition}\nSource: {source_url}\n\nReturn JSON with: title_en, description_en (mention drape, hand-feel, luster, end uses like bridal/evening/luxury home), meta_title_en, meta_description_en, fabric_type, gsm, width_cm, moq, price_usd, composition [{material, percentage}], tags_en (include luxury/premium/bridal/evening), image_urls.',
  ...
  },
  {
    ...
    enrichmentSystemPrompt: 'You are a technical textile specialist for natural fibers. Focus on thread count, weave structure, weight (GSM), shrinkage, certifications (GOTS, Oeko-Tex), sustainability metrics. MANDATORY: NEVER return null for title_en, description_en, usage_en, tags_en, meta_title_en, meta_description_en, or image_alt_en. Generate rich B2B catalog content if raw text is brief. Always respond with valid JSON only.',
    enrichmentUserTemplate: 'Analyze this natural fiber fabric:\n\nRaw title: {raw_title}\nRaw description: {raw_description}\nFabric type: {fabric_type}\nGSM: {gsm}\nComposition: {composition}\nSource: {source_url}\n\nReturn JSON with: title_en (include fiber, weave, weight), description_en (thread count, GSM, shrinkage %, certifications, sustainability, end uses like shirting/bedding/workwear), meta_title_en, meta_description_en, fabric_type, gsm, width_cm, moq, price_usd, composition [{material, percentage}], tags_en (natural/sustainable/breathable/certified), image_urls.',
  ...
  },
  {
    ...
    enrichmentSystemPrompt: 'You are a performance textile specialist for synthetics and blends. Focus on: denier, filament count, stretch %, recovery, moisture-wicking, abrasion resistance, cost-efficiency. MANDATORY: NEVER return null for title_en, description_en, usage_en, tags_en, meta_title_en, meta_description_en, or image_alt_en. Generate rich B2B catalog content if raw text is brief. Always respond with valid JSON only.',
    enrichmentUserTemplate: 'Analyze this performance/synthetic fabric:\n\nRaw title: {raw_title}\nRaw description: {raw_description}\nFabric type: {fabric_type}\nGSM: {gsm}\nComposition: {composition}\nSource: {source_url}\n\nReturn JSON with: title_en (include denier, stretch, application), description_en (polymer type, denier, stretch %/recovery, moisture management, abrasion rating, end uses like sportswear/activewear), meta_title_en, meta_description_en, fabric_type, gsm, width_cm, moq, price_usd, composition [{material, percentage}], tags_en (performance/stretch/moisture-wicking/durable), image_urls.',
