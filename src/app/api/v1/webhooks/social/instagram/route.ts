...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = JSON.parse(rawBody) as { entry?: Array<{ changes?: Array<{ value?: { media_id?: string } }> }> }
    ...
    if (mediaIds.size === 0) return NextResponse.json({ ok: true })
    const db = getDb()
    for (const mediaId of mediaIds) {
      const rows = await db.select({ id: socialPosts.id }).from(socialPosts).where(eq(socialPosts.platformPostId, mediaId)).limit(1)
      const row = rows[0]
      if (row) await enqueueSocialAnalyticsSync(row.id)
    }
    return NextResponse.json({ ok: true, enqueued: mediaIds.size })
  } catch (err) {
    logger.error('Instagram webhook failed', { message: err instanceof Error ? err.message : String(err) })
